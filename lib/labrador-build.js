...
function* build() {
  ...
  let files = getFiles(cwdPath + 'src', '');

  mkdirp(cwdPath + '.build');
  let metadataPath = cwdPath + '.build/metadata.json';
  let metadata = {};
  if (util.isFile(metadataPath)) {
  ...
  for (let file of files) {
    switch (file.ext) {
      case '.js':
        yield buildJS(file.file, dist + file.path, targets, metadata);
        break;
      case '.less':
