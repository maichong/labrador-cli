 ...
 */
const build = (isUglify) => function * buildJS(from, to, ignores) {
  ...
  code = "'use strict';\n" + code;
  if(isUglify){
    code = yield Promise.resolve(uglify.minify(code, {
      fromString: true,
      mangle: {
...
const buildJS = build(false)
buildJS.buildAndUglifyJs = build(true)
