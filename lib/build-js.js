...
const distPath = process.cwd() + '/' + process.env.DIST + '/';
const pagesPath = process.cwd() + '/' + process.env.DIST + '/pages/';
const npmPath = process.cwd() + '/' + process.env.DIST + '/npm/';
const modulesPath = process.cwd() + '/' + process.env.WORKDIR + '/node_modules/';
const cwdPath = process.cwd() + '/' +  + process.env.WORKDIR + '/';
...
function isChanged(from, to, metadata) {
  ...
  if (!metadata[from] || !metadata[to] || metadata[from].mtime !== fromTime || !metadata[from].depends || !util.isFile(to)) {
    metadata[from] = {
      mtime: fromTime
    };
    metadata[to] = {};
  ...
  let toTime = util.getModifiedTime(to).toString();
...
module.exports = function* buildJS(from, to, targets, metadata) {
  ...
  const relativePath = path.relative(distPath, to).replace(/\\/g, '/');
  ...
  if (!isTest && process.env.TEST) {
    ...
    if (util.isFile(file)) {
      ...
      relativeTestPath = path.relative(cwdPath + '/' + process.env.SRC, file);
    }
  }

  //babel转码
  const babel = require(process.cwd() + '/' + process.env.WORKDIR + '/node_modules/babel-core');
  ...
  code = code.replace(/require\(['"]([\w\d_\-\.\/]+)['"]\)/ig, function (match, lib) {
    ...
    if (lib.indexOf('/') === -1 || lib.indexOf('/') === lib.length - 1) {
      ...
      if (!targets[target]) {
        depends[path.normalize(source)] = path.normalize(target);
  ...
  metadata[to].mtime = util.getModifiedTime(to).toString();
  metadata[to].v = version;
