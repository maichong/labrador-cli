...
module.exports = function* buildJS(from, to, targets, metadata) {
  ...
  const relativePath = to.fromDist.replace(/\\/g, '/');
  ...
  let code = babel.transformFileSync(from.file, {
  ...
  code = code.replace(/require\(['"]([\w\d_\-\.\/]+)['"]\)/ig, function (match, lib) {
    ...
    if (lib.indexOf('/') === -1 || lib.indexOf('/') === lib.length - 1) {
      ...
      let pkg = utils.readJSON(path.join(config.modulesDir, lib, '/package.json'));
      let main = pkg.main || 'index.js';
      if (pkg.browser && typeof pkg.browser === 'string') {
        main = pkg.browser;
      }
      let source = path.join(config.modulesDir, lib, main).replace(/\\/g, '/');
      ...
      let target = path.join(distNpmDir, path.relative(config.modulesDir, source));
      relative = path.relative(to.dir, target).replace(/\\/g, '/');
      if (!targets[target]) {
      ...
      let source = config.modulesDir + lib;
      let target = distNpmDir + lib;
      if (lib[0] === '.') {
      ...
      relative = path.relative(to.dir, target);
      if (!targets[target]) {
        depends[source] = target;
        promises.push(buildJS(source, target, targets, metadata));
      }
