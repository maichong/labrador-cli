...
const distPath = path.join(process.cwd(), process.env.DIST) + '/';
const pagesPath = path.join(process.cwd(), process.env.DIST + '/pages') + '/';
...
function minifyApp(appNameMap, appContentMap) {
  ...
  return function () {
    ...
    console.log('minify'.green, path.normalize(process.env.DIST + '/app.wxss').blue);
