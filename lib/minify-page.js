...
function minifyApp(appNameMap, appContentMap) {
  ...
  minified = minified.replace(/::(after|before)/g, ':$1');
  let finalCssContent = '';
  minified.split('\n').forEach((line) => {
  ...
  return function () {
    ...
    for (let key in appNameMap) {
      let matchs = key.match(/(.*):+(after|before)$/);
...
function minify(page, appNameMap, appContentMap) {
  ...
  if (cssContent) {
    ...
    cssContent = cssContent.replace(/::(after|before)/g, ':$1');
  }
  // LESS中存在的className列表
  let styleClassNames = {};
  ...
  xmlContent = xmlContent.replace(/ class="([^"]+)"/g, (matchs, names) => {
    ...
    names = names.split(' ').filter((name) => {
      ...
      if (config.classNames && config.classNames[name]) {
        ...
        } else if (appNameMap[name + ':before'] || appNameMap[name + ':after']) {
      ...
      if (appNameMap[name]) {
        ...
        id = createId();
        // 如果app.less中存在 .name:before 或 .name:after 但是不存在 .name，自动添加 .name
        if (appNameMap[name + ':before'] || appNameMap[name + ':after']) {
