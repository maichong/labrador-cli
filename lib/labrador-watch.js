...
function* watch() {
    ...
    switch (file.ext) {
      ...
      case '.less':
        if ((!file.dir && file.base === 'app.less') || file.dir.startsWith('pages')) {
          ...
          console.log('changed'.green, path.relative(cwdPath, source).blue);
          let files = Object.keys(refs[source]);
          for (let s of files) {
            yield* buildFile(s);
          }
        } else {
      ...
      case '.xml':
        if (file.dir.startsWith('pages')) {
          ...
          let depends = yield* buildXML(source, to);
          depends.forEach((d) => {
            d = path.normalize(d);
            if (!refs[d]) {
              refs[d] = {};
  ...
  chokidar.watch(src, { ignored: /^\./ }).on('all', (event, source) => {
    ...
    co(buildFile(source)).catch((error) => {
      console.log(error.message, error.codeFrame || '');
    });
  });
}
