...
program
  ...
  .option('-i, --dist [path]', '生成目录, 默认值:dist')
...
const src = process.cwd() + '/' + process.env.SRC + '/';
const dist = process.cwd() + '/' + process.env.DIST + '/';
    ...
    switch (file.ext) {
      ...
      case '.less':
        if ((!file.dir && file.base === 'app.less') || file.dir.startsWith('pages')) {
          ...
          console.log('ignore'.yellow, path.join(process.env.SRC, relative).gray);
        }
        break;
      case '.xml':
